...
def coprocess_debug(instructions):
    curr = 0
    queue = []
    ...
    operations = {
        "set": lambda x, y: registers[y] if y in registers else int(y),
        "sub": lambda x, y: register[x] - register[y] if y in register else register[x] - int(y),
        "mul": lambda x, y: register[x] * register[y] if y in register else register[x] * int(y),
        "jgz": lambda x, y: print(f"(Program #{r}) Jumping {y} instructions!") if x > 0 else print(f"(Program #{r}) Jump instruction ignored!"),
    }
