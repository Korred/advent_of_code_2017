...
def clean(stream):
    ...
    # count characters in garbage and then remove all garbage
    pat1 = re.compile(r'<.*?>')
    pat2 = re.compile(r'{,')
    pat3 = re.compile(r',}')
    
    #garbage_char_cnt = len("".join(re.findall(pat1, text_stream)).replace("<","").replace(">",""))
    garbage_char_cnt = "".join(re.findall(pat1, text_stream)).replace('<','')


    print(garbage_char_cnt)
